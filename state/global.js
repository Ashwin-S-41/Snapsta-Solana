...
export const GlobalState = ({ children }) => {
  ...
  const fetchLikers = useCallback(async () => {
    if (!program) return;
    const likers = await program.account.like.all();
    console.log("getting likerss...", likers)
    setLikers(likers.map((liker) => liker.account));
  }, [program]);
   console.log(likers)
  // Get and update likers
  useEffect(() => {
    if (!likers) {
      // Fetch likers if posts don't exists
      fetchLikers();
    }
  }, [likers, fetchLikers]);
  ...
  }, [program]);// Added wallet as a dependency
